# agg SNOTEL like MODIS
import os, glob
import pandas as pd
import numpy as np

base_path = '/workspace/Shared/Tech_Projects/SERDP_Fish_Fire/project_data'
snotel_path = os.path.join(base_path,'STATION_DATA','snotel','prepped')
output_path = os.path.join(base_path,'STATION_DATA','snotel','8day')
...
# list the snotel files that have been output:
files = glob.glob(os.path.join(snotel_path, '*.csv'))
sensor = 'MOD11A2' # only use the longest one, since the dates are the same
